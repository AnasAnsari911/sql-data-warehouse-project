...
BEGIN 
	...
	BEGIN TRY
		...
		PRINT 'Load duration for table ' + OBJECT_NAME(OBJECT_ID('bronze.crm_cust_info')) + ' is ' + CAST(DATEDIFF(SECOND,@start_time, @end_time) as NVARCHAR)     + ' seconds';
		...
		PRINT 'Load duration for table ' + OBJECT_NAME(OBJECT_ID('bronze.crm_prd_info')) + ' is ' + CAST(DATEDIFF(SECOND,@start_time, @end_time) as NVARCHAR)     + ' seconds';
		...
		PRINT 'Load duration for table ' + OBJECT_NAME(OBJECT_ID('bronze.crm_sales_details')) + ' is ' + CAST(DATEDIFF(SECOND,@start_time, @end_time) as NVARCHAR)     + ' seconds';
		...
		PRINT 'Load duration for table ' + OBJECT_NAME(OBJECT_ID('bronze.erp_cust_az12')) + ' is ' + CAST(DATEDIFF(SECOND,@start_time, @end_time) as NVARCHAR)     + ' seconds';
		...
		PRINT 'Load duration for table ' + OBJECT_NAME(OBJECT_ID('bronze.erp_loc_a101')) + ' is ' + CAST(DATEDIFF(SECOND,@start_time, @end_time) as NVARCHAR)     + ' seconds';

		SET @end_time = GETDATE()
		PRINT '>> Truncating Table : bronze.erp_px_cat_v1g2 ';
